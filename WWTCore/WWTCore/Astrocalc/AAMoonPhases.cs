...
//Static methods
  ...
  public static double TruePhase(double k)
  {
	...
	var M = CAACoordinateTransformation.MapTo0To360Range(2.5534 + 29.10535670 *k - 0.0000014 *T2 - 0.00000011 *T3);
	M = CAACoordinateTransformation.DegreesToRadians(M);
	var Mdash = CAACoordinateTransformation.MapTo0To360Range(201.5643 + 385.81693528 *k + 0.0107582 *T2 + 0.00001238 *T3 - 0.000000058 *T4);
	Mdash = CAACoordinateTransformation.DegreesToRadians(Mdash);
	var F = CAACoordinateTransformation.MapTo0To360Range(160.7108 + 390.67050284 *k - 0.0016118 *T2 - 0.00000227 *T3 + 0.00000001 *T4);
	F = CAACoordinateTransformation.DegreesToRadians(F);
	var omega = CAACoordinateTransformation.MapTo0To360Range(124.7746 - 1.56375588 *k + 0.0020672 *T2 + 0.00000215 *T3);
	omega = CAACoordinateTransformation.DegreesToRadians(omega);
	var A1 = CAACoordinateTransformation.MapTo0To360Range(299.77 + 0.107408 *k - 0.009173 *T2);
	A1 = CAACoordinateTransformation.DegreesToRadians(A1);
	var A2 = CAACoordinateTransformation.MapTo0To360Range(251.88 + 0.016321 *k);
	A2 = CAACoordinateTransformation.DegreesToRadians(A2);
	var A3 = CAACoordinateTransformation.MapTo0To360Range(251.83 + 26.651886 *k);
	A3 = CAACoordinateTransformation.DegreesToRadians(A3);
	var A4 = CAACoordinateTransformation.MapTo0To360Range(349.42 + 36.412478 *k);
	A4 = CAACoordinateTransformation.DegreesToRadians(A4);
	var A5 = CAACoordinateTransformation.MapTo0To360Range(84.66 + 18.206239 *k);
	A5 = CAACoordinateTransformation.DegreesToRadians(A5);
	var A6 = CAACoordinateTransformation.MapTo0To360Range(141.74 + 53.303771 *k);
	A6 = CAACoordinateTransformation.DegreesToRadians(A6);
	var A7 = CAACoordinateTransformation.MapTo0To360Range(207.14 + 2.453732 *k);
	A7 = CAACoordinateTransformation.DegreesToRadians(A7);
	var A8 = CAACoordinateTransformation.MapTo0To360Range(154.84 + 7.306860 *k);
	A8 = CAACoordinateTransformation.DegreesToRadians(A8);
	var A9 = CAACoordinateTransformation.MapTo0To360Range(34.52 + 27.261239 *k);
	A9 = CAACoordinateTransformation.DegreesToRadians(A9);
	var A10 = CAACoordinateTransformation.MapTo0To360Range(207.19 + 0.121824 *k);
	A10 = CAACoordinateTransformation.DegreesToRadians(A10);
	var A11 = CAACoordinateTransformation.MapTo0To360Range(291.34 + 1.844379 *k);
	A11 = CAACoordinateTransformation.DegreesToRadians(A11);
	var A12 = CAACoordinateTransformation.MapTo0To360Range(161.72 + 24.198154 *k);
	A12 = CAACoordinateTransformation.DegreesToRadians(A12);
	var A13 = CAACoordinateTransformation.MapTo0To360Range(239.56 + 25.513099 *k);
	A13 = CAACoordinateTransformation.DegreesToRadians(A13);
	var A14 = CAACoordinateTransformation.MapTo0To360Range(331.55 + 3.592518 *k);
	A14 = CAACoordinateTransformation.DegreesToRadians(A14);
  
	//convert to radians
	double kint=0;
    var kfrac = GlobalMembersStdafx.modf(k, ref kint);
	if (kfrac < 0)
	  kfrac = 1 + kfrac;
	if (kfrac == 0) //New Moon
	{
	  var DeltaJD = -0.40720 *Math.Sin(Mdash) + 0.17241 *E *Math.Sin(M) + 0.01608 *Math.Sin(2 *Mdash) + 0.01039 *Math.Sin(2 *F) + 0.00739 *E *Math.Sin(Mdash - M) + -0.00514 *E *Math.Sin(Mdash + M) + 0.00208 *E2 *Math.Sin(2 *M) + -0.00111 *Math.Sin(Mdash - 2 *F) + -0.00057 *Math.Sin(Mdash + 2 *F) + 0.00056 *E *Math.Sin(2 *Mdash + M) + -0.00042 *Math.Sin(3 *Mdash) + 0.00042 *E *Math.Sin(M + 2 *F) + 0.00038 *E *Math.Sin(M - 2 *F) + -0.00024 *E *Math.Sin(2 *Mdash - M) + -0.00017 *Math.Sin(omega) + -0.00007 *Math.Sin(Mdash + 2 *M) + 0.00004 *Math.Sin(2 *Mdash - 2 *F) + 0.00004 *Math.Sin(3 *M) + 0.00003 *Math.Sin(Mdash + M - 2 *F) + 0.00003 *Math.Sin(2 *Mdash + 2 *F) + -0.00003 *Math.Sin(Mdash + M + 2 *F) + 0.00003 *Math.Sin(Mdash - M + 2 *F) + -0.00002 *Math.Sin(Mdash - M - 2 *F) + -0.00002 *Math.Sin(3 *Mdash + M) + 0.00002 *Math.Sin(4 *Mdash);
	  JD += DeltaJD;
	}
