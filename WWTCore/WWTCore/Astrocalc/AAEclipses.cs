...
//Static methods
  ...
  public static CAALunarEclipseDetails CalculateLunar(double k)
  {
	...
	Debug.Assert(!bSolarEclipse);
      ...
      double Mdash = 0;
	var solarDetails = Calculate(k, ref Mdash);
  
	//What will be the return value
	var details = new CAALunarEclipseDetails();
  ...
  protected static CAASolarEclipseDetails Calculate(double k, ref double Mdash)
  {
	...
	double intp = 0;
    var bSolarEclipse = (GlobalMembersStdafx.modf(k, ref intp) == 0);
  
	//What will be the return value
	var details = new CAASolarEclipseDetails();
	...
	details.F = F;
	var Fdash = F - 0.02665 *Math.Sin(omega);
	...
	var P = 0.2070 *E *Math.Sin(M) + 0.0024 *E *Math.Sin(2 *M) + -0.0392 *Math.Sin(Mdash) + 0.0116 *Math.Sin(2 *Mdash) + -0.0073 *E *Math.Sin(Mdash + M) + 0.0067 *E *Math.Sin(Mdash - M) + 0.0118 *Math.Sin(2 *Fdash);
  
	var Q = 5.2207 + -0.0048 *E *Math.Cos(M) + 0.0020 *E *Math.Cos(2 *M) + -0.3299 *Math.Cos(Mdash) + -0.0060 *E *Math.Cos(Mdash + M) + 0.0041 *E *Math.Cos(Mdash - M);
